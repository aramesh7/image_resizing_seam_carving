function out = change_aspect_forward(im, new_n, new_m, energy_type)
[n, m, ~] = size(im);
    out = zeros();
    ...
    if c>0 && r>0
        ...
        for j = 1:c+1
            ...
            for i = 1:r+1
                ...
                if i-1 <= 0
                    ...
                    [vert_seam,cost] = forward_vertical_seam(curr);
                ...
                [vert_seam,vcost] = forward_vertical_seam(cell2mat(last_col_vec(i)));
        ...
        for i = 1:size(ops)
            if ops(i) == 0 %vertical
                ...
                [vert_seam,~] = forward_vertical_seam(iterate_im);
                iterate_im = cut(iterate_im, vert_seam, 'vertical');
                imshow(iterate_im)
            else
                E = energy(iterate_im, energy_type);
                [horz_seam,~] = horizontal_seam(iterate_im);
    ...
    elseif r > 0
        ...
        for i = 1:r %cut horizontal seam r times
            E = energy(iterate_im, energy_type);
            [horz_seam,~] = forward_horizontal_seam(iterate_im);
            iterate_im = cut(iterate_im, horz_seam, 'horizontal');
            figure(3),imshow(iterate_im);
        end
