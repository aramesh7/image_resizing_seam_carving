...
function out = change_aspect_gradient(im, new_n, new_m, energy_type)
    [n, m, ~] = size(im);
    ...
    elseif c > 0
        ...
        for i = 1:c %cut vertical seam c times
            i
            E = energy(iterate_im, energy_type);
            [Fxr,Fyr] = gradient(im2double(iterate_im(:,:,1)));
            [Fxg,Fyg] = gradient(im2double(iterate_im(:,:,2)));
            [Fxb,Fyb] = gradient(im2double(iterate_im(:,:,3)));
            [vert_seam,~] = vertical_seam(iterate_im, E);
